 ...
 *
 * Classe adaptée à l'ouverture d'une connexion bd avec PDO pour DB2 sur serveur IBM i
 */
...
class PDO_DB2IBMi_DBInstance implements DBInstanceInterface {
    ...
    public function __construct($system, $user, $password, $options = array(), $persistent = false) {
        ...
        // pr�paration du tableau des options au format attendu par le connecteur DB2
        ...
        try {
            ...
            error_log('FATAL ERROR : PDOException sur connexion DB dans la m�thode ' . __METHOD__ . ' de la classe ' . __CLASS__);
            error_log('FATAL ERROR : ' . $e->getMessage());
        } catch (Exception $e) {
            error_log('FATAL ERROR : Exception sur connexion DB dans la m�thode ' . __METHOD__ . ' de la classe ' . __CLASS__);
            error_log('FATAL ERROR : ' . $e->getMessage());
        }
